...
double NewPing::ping_cm_mm(unsigned int max_cm_distance) {
  unsigned long echoTime = NewPing::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
  return ((double)echoTime / (double)US_ROUNDTRIP_CM);              // Call the ping method and returns the distance in centimeters (no rounding).
}
